...
<div class="page-header mb-4">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            ...
            <p class="text-muted">Gérez les clubs de l'école</p>
        </div>
        <a href="{{ route('admin.clubs.create') }}" class="btn btn-success">
            <i class="fas fa-plus me-2"></i>Ajouter un club
        </a>
    </div>
</div>
...
<x-admin.table
    ...
    icon="fas fa-list"
    :headers="['Nom', 'Catégorie', 'Membres', 'Date de création']"
>
        ...
        <tr>
            <td>
                <div class="d-flex align-items-center">
                    @if($club->image)
                        <img src="{{ asset('storage/' . $club->image) }}" alt="{{ $club->name }}" class="rounded-circle" width="40" height="40">
                    @else
            ...
            <td class="text-end">
                <div class="actions">
                    <a href="{{ route('admin.clubs.edit', $club) }}" class="btn btn-success btn-sm" title="Modifier">
                        <i class="fas fa-edit"></i>
                    </a>
                    <form action="{{ route('admin.clubs.destroy', $club) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce club ?')" title="Supprimer">
        ...
        <tr>
            <td colspan="5">
                <div class="empty-message">
                    ...
                    <a href="{{ route('admin.clubs.create') }}" class="btn btn-success mt-3">
                        <i class="fas fa-plus me-2"></i>Ajouter un club
                    </a>
                </div>
            </td>
